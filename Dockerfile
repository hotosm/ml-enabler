...
CMD service nginx restart \
    ...
    && echo "INSERT INTO users (name, password) VALUES ('machine', crypt('$MACHINE_AUTH', gen_salt('bf', 10)))" || true \
    && echo "UPDATE users SET password = crypt('$MACHINE_AUTH', gen_salt('bf', 10)) WHERE name = 'machine'" || true \
